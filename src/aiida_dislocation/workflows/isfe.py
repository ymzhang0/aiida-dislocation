...
class ISFEWorkChain(SFEBaseWorkChain):
    ...
    def should_run_sfe(self):
        ...
        self.ctx.intrinsic_multiplier = self._calculate_structure_multiplier(actual_structure)
    ...
    def run_sfe(self):
        ...
        inputs.metadata.call_link_label = f'isfe_{self.ctx.iteration}'
        fault_method = self.inputs.fault_method.value.lower() if self.inputs.fault_method.value else 'removal'
    ...
    def inspect_sfe(self):
        """Inspect the SFE calculation for intrinsic stacking fault."""
        ...
        exit_code = self._inspect_workchain(
            workchain,
            'PwRelaxWorkChain',
            'intrinsic faulted geometry',
            self.exit_codes.ERROR_SUB_PROCESS_FAILED_ISF,
        ...
        # Extract energy
        total_energy_isf_geometry = self._get_workchain_energy(workchain)
        
        # Report energy
        self._report_energy(
            total_energy_isf_geometry,
            self.ctx.intrinsic_multiplier,
            'intrinsic faulted geometry',
            'unit cells'
        )
        ...
        if not hasattr(self.ctx, 'isfe_data'):
            self.ctx.isfe_data = []
        
        self.ctx.isfe_data.append({
            'spacing': self.ctx.current_spacing,
            'iteration': self.ctx.iteration - 1,
            'energy_ry': float(total_energy_isf_geometry),
            'intrinsic_multiplier': self.ctx.intrinsic_multiplier,
            'isfe_j_m2': (
